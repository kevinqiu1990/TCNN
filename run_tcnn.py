...
for path in path_train_and_test:
    ...
    for batch_size in opt_batchsize:
        ...
        for params_i in itertools.product(opt_node, opt_epoch, opt_learning_rate, opt_lambda, opt_gamma):
            ...
            # ------------------ CNN training begins ------------------
            TCNN_acc, TCNN_auc, TCNN_f1, TCNN_mcc = [], [], [], []
            DPTCNN_acc, DPTCNN_auc, DPTCNN_f1, DPTCNN_mcc = [], [], [], []
            for l in range(LOOP_SIZE):
                model = TCNN.TCNN(nn_params)
                model.to(DEVICE)
            ...
            calculate_save_data('DPTCNN', path[0], path[1], nn_params, mmd_params, LOOP_SIZE, DPTCNN_acc, DPTCNN_auc, DPTCNN_f1, DPTCNN_mcc)
            # ------------------ Training End ------------------
